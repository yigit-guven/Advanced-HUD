	...
	private class PreparedTextBuilder implements Font.PreparedText, FormattedCharSink {
		...
		public boolean accept(final int position, final Style style, final BakedGlyph glyph) {
			...
			if (style.isStrikethrough()) {
				this.addEffect(
					Font.this.provider.effect().createEffect(effectX0, this.y + 4.5F - 1.0F, this.x + advance, this.y + 4.5F, 0.01F, textColor, shadowColor, shadowOffset)
				);
			}

			if (style.isUnderlined()) {
				this.addEffect(
					Font.this.provider.effect().createEffect(effectX0, this.y + 9.0F - 1.0F, this.x + advance, this.y + 9.0F, 0.01F, textColor, shadowColor, shadowOffset)
				);
			}
